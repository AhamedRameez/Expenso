// lib/screens/debit_detail_screen.dart
import 'dart:io';
...
import 'debit_edit_screen.dart';

class DebitDetailScreen extends StatefulWidget {
  final Map<String, dynamic> debit;

  const DebitDetailScreen({super.key, required this.debit});
...
class _DebitDetailScreenState extends State<DebitDetailScreen> {
  ...
  Future<void> _reloadDebit() async {
  ...
  Future<void> _markAsFullyPaid() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('This will mark the entire debt as paid.'),
        actions: [
          TextButton(
    ...
    final totalAmount = (_debit['amount'] as num).toDouble();
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                try {
                  ...
                  setState(() {
                    _debit['paidAmount'] = newPaidAmount;
                  ...
                  if (mounted) {
                    ...
                    _showSnackBar(
                      newStatus == 'paid'
                          ? 'Fully paid!'
                          : '₹${NumberFormat('#,###').format(amount)} paid!',
                      Colors.green,
                    );
                  }
                } catch (e) {
  ...
  Future<void> _deleteDebit() async {
    final confirm = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
  ...
  Future<void> _exportPDF() async {
    ...
    try {
      ...
      final name = _debit['personName'] as String? ?? 'Unknown';
      final totalAmount = (_debit['amount'] as num?)?.toDouble() ?? 0;
      final paidAmount = (_debit['paidAmount'] as num?)?.toDouble() ?? 0;
      final remaining = totalAmount - paidAmount;
      final status = _debit['status'] as String? ?? 'pending';
      final dateBorrowed = _debit['dateBorrowed'] as Timestamp?;
      final dueDate = _debit['dueDate'] as Timestamp?;
      final pc = PdfColor.fromInt(0xFFE74C3C),
              ...
              child: pw.Column(
                ...
                children: [
                  ...
                  _pdfRow(
                    'Date Borrowed',
                    dateBorrowed != null
                        ? df.format(dateBorrowed.toDate())
                        : '-',
                    dc,
                  ),
                  if (dueDate != null)
                    _pdfRow('Due Date', df.format(dueDate.toDate()), dc),
                  pw.Divider(color: PdfColor.fromInt(0xFFE2E8F0)),
                  _pdfRow('Total Amount', cf.format(totalAmount), dc),
                  _pdfRow('Amount Paid', cf.format(paidAmount), grc),
                  _pdfRow(
                    'Remaining',
                    cf.format(remaining),
                    remaining > 0 ? rc : grc,
      ...
      final fileName =
          'Debit_Report_${DateFormat('yyyyMMdd_HHmmss').format(DateTime.now())}.pdf';
  ...
  Widget build(BuildContext context) {
    ...
    final status = _debit['status'] as String? ?? 'pending';
    final contact = _debit['contact'] as String? ?? '';
    final notes = _debit['notes'] as String? ?? '';
    final dateBorrowed = _debit['dateBorrowed'] as Timestamp?;
    final dueDate = _debit['dueDate'] as Timestamp?;
    final isOverdue =
        status != 'paid' &&
    ...
    if (status == 'paid') {
      ...
      statusColor = const Color.fromARGB(255, 56, 104, 67);
          ...
          IconButton(
            ...
            onPressed: () async {
              final result = await Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => DebitEditScreen(debit: _debit),
                ),
              );
              if (result == true) _reloadDebit();
            },
          ),
          IconButton(
            icon: const Icon(
              ...
              child: Column(
                children: [
                    ...
                    Container(
                      ...
                      child: Column(
                        children: _paymentHistory.map((p) {
                          ...
                          final pDate = p['date'] as Timestamp?;
                          final isAddedMoney =
                              (p['type'] as String? ?? '') == 'added_money';
                          return ListTile(
                            leading: Container(
                    ...
                    child: Column(
                      children: [
                        ...
                        _detailRow(
                          ...
                          'Borrowed',
                          dateBorrowed != null
                              ? dateFormat.format(dateBorrowed.toDate())
                              : '-',
                        ),
                        if (dueDate != null) ...[
                          const Divider(height: 16),
                          _detailRow(
                            Icons.event,
                            'Due Date',
                            dateFormat.format(dueDate.toDate()),
                  ...
                  if (amount > 0) ...[
                    ...
                    Text(
                      '${((paidAmount / amount) * 100).toStringAsFixed(0)}% paid',
                      style: TextStyle(
                        fontSize: 11,
